...
# ----------------------------------------------------------------------------
CORE = {
    "GDP":    ("GDPC1",    "yoy"),      # real GDP, YoY% (deck convention)
    "INF":    ("PCEPILFE", "yoy"),      # core PCE, YoY% (deck convention)
    "POLICY": ("FEDFUNDS", "level"),    # effective fed funds rate
    "FX":     ("RBUSBIS",  "yoy"),      # REER YoY% (Temasek deck definition); binds sample to 1995+
}
...
def transform(level, kind):
    ...
    if kind == "yoy":    return 100.0 * (level / level.shift(4) - 1.0)   # %, YoY (deck convention)
...
if len(eligible) == 0:
    print("\n[note] NO spec passes hard filters: YoY (4-qtr overlapping) flows inject "
          "MA-type residual autocorrelation, so the whiteness test fails for EVERY spec. "
          "Falling back to rank all specs for reporting only; none are clean.")
    eligible = tab.sort_values("avg_rank_h1").copy()
...
csv_path = OUT / "var_horserace_results_allyoy.csv"
...
for f in ["relrmse_heatmap.png", "winner_forecasts.png"]:
    (OUT / f.replace(".png", "_allyoy.png")).write_bytes((FIG / f).read_bytes())
print(f"[saved] {OUT/'relrmse_heatmap_allyoy.png'}\n[saved] {OUT/'winner_forecasts_allyoy.png'}")
...
(OUT / "var_horserace_summary_allyoy.json").write_text(json.dumps(summary, indent=2))
print(f"[saved] {OUT/'var_horserace_summary_allyoy.json'}")
print("\nDONE.")
